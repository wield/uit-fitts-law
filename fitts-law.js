  ...
  if (content.length) {
    ...
    for (const row of rows) {
      ...
      for (let c = 0; c < row.length; c++) {
        ...
        if (c == (row.length - 1) || (char == ',' && !escaped)) {
          if (c == (row.length - 1)) {
            value += char
          }
          ...
          if (index == 0) {
            ...
            try {
              ...
              if (!result) result = {}
              result[headers[column]] = JSON.parse(value)
            } catch (error) {
              if (!result) result = {}
  ...
  addDataPoint: function (data) {
    ...
    if (dt < MAX_TIME)  // skip if obvious outlier
    {
      ...
      this.data[this.currentDataSet].data.push({
        time: dt, distance: data.target.distance, width: data.target.w, hit: data.hit,
...
$('#importDataSetButton').click(function () {
  let data
  ...
  input.change(function () {
    ...
    if (file) {
      ...
      reader.onload = function (evt) {
        data = fromCSV(evt.target.result)

        fittsTest.data[fittsTest.currentDataSet].data = JSON.parse(JSON.stringify(data))
        fittsTest.addDataSet()
      };
